...
update_waybar_config() {
  ...
  MAIN_ID=$(get_output_id "$MAIN_DISPLAY")
  SECONDARY_ID=$(get_output_id "$SECONDARY_DISPLAY")
  MINI_ID=$(get_output_id "$MINI_DISPLAY")
  LAPTOP_ID=$(get_output_id "$LAPTOP_OUTPUT")

  case "$1" in
  close)
    json=$(jq -n \
      --arg main "$MAIN_ID" \
      --arg sec "$SECONDARY_ID" \
      --arg mini "$MINI_ID" '
        {
          "1": [$main],
          "2": [$main],
